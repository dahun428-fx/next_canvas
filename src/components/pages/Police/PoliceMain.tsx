...
import { mergeByCity, seperateByCity } from "@/utils/openapi/police/police";
import { TabContext, TabList, TabPanel } from "@mui/lab";
import { Box, Card, CardHeader, FormControl, Grid, InputLabel, MenuItem, Select, SelectChangeEvent, Tab, Typography } from "@mui/material";
...
export const PoliceMain: React.FC<Props> = ({ onReload, searchPoliceRequest, policeResponse, policeYears }) => {
  ...
  return (
    <>
      ...
      <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
        ...
        <Select labelId="demo-select-small-label" id="demo-select-small" value={yearValue} label="연도" onChange={(e) => handleChange(e)}>
          {policeYears &&
      ...
      <TabContext value={tabValue}>
        <Box sx={{ borderBottom: 1, borderColor: "divider" }}>
          ...
          >
            {seperatedPoliceList && seperatedPoliceList.length > 0 && <Tab label={`전체`} value={`police_all`} />}
            {seperatedPoliceTabNames &&
              seperatedPoliceTabNames.length > 0 &&
              seperatedPoliceTabNames.map((tabName, tabId) => {
                return <Tab key={`police_${tabId}`} label={tabName} value={`police_${tabId}`} />;
              })}
          </TabList>
        </Box>
          ...
          <Grid container spacing={3}>
            {mergePoliceList &&
              ...
              mergePoliceList.map((item, index) => {
                ...
                return <PoliceItem key={`all_${index}`} dataObj={dataObj} title={city} subheader={subheader} />;
              })}
          </Grid>
        </TabPanel>
        {seperatedPoliceList &&
