...
function isOpenApiErrorResponse(
	response: unknown
): response is OpenApiErrorResponse {
	if (response == null) {
		return true;
	}
...
const generateApi = () => {
	...
	async function get<T extends OpenApiRequest, R extends OpenApiResponse>(
		url: string,
		request?: T,
	...
	async function post<T extends OpenApiRequest, R extends OpenApiResponse>(
		url: string,
		request: T,
		options?: RequestOption
	): Promise<R> {
	...
	async function put<T extends OpenApiRequest, R extends OpenApiResponse>(
		url: string,
		request: T,
		options?: RequestOption
	): Promise<R> {
		return $axios
			.put(url, request, { cancelToken: options?.cancelToken })
			.then(response => response.data);
	}

	async function _delete<T extends OpenApiRequest, R extends OpenApiResponse>(
		url: string,
		request?: T,
		options?: RequestOption
	): Promise<R> {
